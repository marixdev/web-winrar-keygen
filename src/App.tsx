...
function App() {
  const [page, setPage] = useState<Page>("keygen");
        ...
        <div className="app-shell">
          <header className="top-bar">
            ...
            <nav className="nav-tabs">
              <button
                className={`nav-tab ${page === "keygen" ? "nav-tab--active" : ""}`}
                onClick={() => setPage("keygen")}
              >
                <svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8 7a5 5 0 113.61 4.804l-1.903 1.903A1 1 0 019 14H8v1a1 1 0 01-1 1H6v1a1 1 0 01-1 1H3a1 1 0 01-1-1v-2a1 1 0 01.293-.707L8.196 8.39A5.002 5.002 0 018 7zm5-3a.75.75 0 000 1.5A1.5 1.5 0 0114.5 7 .75.75 0 0016 7a3 3 0 00-3-3z" clipRule="evenodd" /></svg>
                {t.navKeygen}
              </button>
              <button
                className={`nav-tab ${page === "how-it-works" ? "nav-tab--active" : ""}`}
                onClick={() => setPage("how-it-works")}
              >
                <svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clipRule="evenodd" /></svg>
                {t.navHowItWorks}
              </button>
            </nav>
          </header>
          <main className="main-content">
            {page === "keygen" ? <KeygenForm /> : <HowItWorks />}
          </main>
          <footer className="app-footer">
            <p className="disclaimer">📚 {t.disclaimerText}</p>
            <p>{t.footerText}</p>
